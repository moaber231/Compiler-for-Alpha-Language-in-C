...
void show_hidden(SymTable_T oSymTable)
{
	...
	for(i=0;i<SIZE;i++)
	{
		...
		while(find!=NULL)
		{
			if(find->type==USERFUNC || find->type==LIBFUNC)
			{
				if(find->isActive==false)printf("Hidden Function: %s\n",find->value.funcVal->name);
			}
			else
			{
				if(find->isActive==false)printf("Hidden Variable: %s\n",find->value.varVal->name);
			} 
			find=find->next;
		}
	}
...
void print_all(SymTable_T oSymTable)
{
	...
	for(i=0;i<=oSymTable->max_scope;i++)
	{
		...
		for(int j=0;j<SIZE;j++)
		{
			...
			while(find!=NULL)
			{
				if(find->type==LIBFUNC && find->value.funcVal->scope==i)printf("\"%s\" [LIBFUNC] (line=%d) (scope=%d)\n",find->value.funcVal->name,find->value.funcVal->line,find->value.funcVal->scope);
				find=find->next;
			}
		}
		for(int j=0;j<SIZE;j++)
		{
			...
			while(find!=NULL)
			{
				if(find->type==USERFUNC && find->value.funcVal->scope==i)printf("\"%s\" [USERFUNC] (line=%d) (scope=%d)\n",find->value.funcVal->name,find->value.funcVal->line,find->value.funcVal->scope);
				find=find->next;
			}
		}
		for(int j=0;j<SIZE;j++)
		{
			...
			while(find!=NULL)
			{
				if(find->type==GLOBAL && find->value.varVal->scope==i)printf("\"%s\" [GLOBAL] (line=%d) (scope=%d)\n",find->value.varVal->name,find->value.varVal->line,find->value.varVal->scope);
				find=find->next;
			}
		}
		for(int j=0;j<SIZE;j++)
		{
			...
			while(find!=NULL)
			{
				if(find->type==LOCAL && find->value.varVal->scope==i)printf("\"%s\" [LOCAL] (line=%d) (scope=%d)\n",find->value.varVal->name,find->value.varVal->line,find->value.varVal->scope);
				find=find->next;
			}
		}
		for(int j=0;j<SIZE;j++)
		{
			...
			while(find!=NULL)
			{
				if(find->type==FORMAL && find->value.varVal->scope==i)printf("\"%s\" [FORMAL] (line=%d) (scope=%d)\n",find->value.varVal->name,find->value.varVal->line,find->value.varVal->scope);
				find=find->next;
			}
		}
